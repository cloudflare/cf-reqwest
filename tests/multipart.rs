...
async fn text_part() {
    ...
    let form = cf_reqwest::multipart::Form::new().text("foo", "bar");
...
async fn stream_part() {
    ...
    let stream =
        cf_reqwest::Body::wrap_stream(stream::once(future::ready(Ok::<_, cf_reqwest::Error>(
            "part1 part2".to_owned(),
        ))));
    let part = cf_reqwest::multipart::Part::stream(stream);
