...
impl ClientBuilder {
    ...
    pub fn build(self) -> crate::Result<Client> {
        ...
        let mut connector = {
            ...
            let base_connector = match config.connector {
                ...
                None => {
                    let mut http =
                        HttpConnector::new_with_resolver(DynResolver::new(resolver.clone()));

                    http.set_connect_timeout(config.connect_timeout);

                    BaseConnector::Http(http)
                }
            };
            ...
            if config.enforce_plaintext && config.https_only {
                return Err(error::builder(
                    "`enforce_plaintext` and `https_only` options are mutually exclusive",
                ));
            }

            if config.enforce_plaintext {
                Connector::new(
                    base_connector,
                    proxies.clone(),
                                ...
                                .map_err(crate::error::builder)?
                            {
                                ...
                                match root_cert_store.add(cert.into()) {
                                    ...
                                    Err(err) => {
                                        invalid_count += 1;
                                        log::debug!(
                                            "rustls failed to parse DER certificate: {err:?}"
                                        );
                                    }
                                }
                            }
    ...
    /// Only use HTTP/3.
    #[cfg(feature = "http3")]
    #[cfg_attr(docsrs, doc(cfg(all(cf_reqwest_unstable, feature = "http3",))))]
    ...
    /// Enforce the Client to send plaintext even to HTTPS URLs.
    ///
    /// Defaults to false.
    pub fn enforce_plaintext(mut self, enabled: bool) -> ClientBuilder {
        self.config.enforce_plaintext = enabled;
        self
    }
