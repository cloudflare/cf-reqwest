...
//! # Backends
//!
//! cf-reqwest supports several TLS backends, enabled with Cargo features.
//!
//! ## default-tls
//!
//! cf-reqwest will pick a TLS backend by default. This is true when the
...
//! cf-reqwest to change the default to `rustls` (or another) at some point in the
//! future.
//!
...
impl Certificate {
    ...
    ///     .read_to_end(&mut buf)?;
    /// let cert = cf_reqwest::Certificate::from_der(&buf)?;
    /// # drop(cert);
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///     .read_to_end(&mut buf)?;
    /// let cert = cf_reqwest::Certificate::from_pem(&buf)?;
    /// # drop(cert);
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///     .read_to_end(&mut buf)?;
    /// let certs = cf_reqwest::Certificate::from_pem_bundle(&buf)?;
    /// # drop(certs);
    /// # Ok(())
    /// # }
    /// ```
...
impl Identity {
    ...
    ///     .read_to_end(&mut buf)?;
    /// let pkcs12 = cf_reqwest::Identity::from_pkcs12_der(&buf, "my-privkey-password")?;
    /// # drop(pkcs12);
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///     .read_to_end(&mut buf)?;
    /// let id = cf_reqwest::Identity::from_pem(&buf)?;
    /// # drop(id);
    /// # Ok(())
    /// # }
    /// ```
